...
- Prim算法：随机选一个顶点作为开始，每次连通通往周边节点的边中长度最小的一条边（用[PriorityQueue](#优先队列)存储），把新节点加入疆域，循环往复
...
>
> ![Prim vs Dijkstra](<./img/Prim vs Dijkstra.gif>)
>
...
>
> 但对于数值范围超大，但又极其稀疏的数列（比如`[-114514, 1919810]`，$N=2,\ R=2034324 $，岂不尴尬）

### LSD Radix Sort

从低位到高位，进行CountingSort

$N$个记录，每个记录有$D$个关键字（$D$是最长元素的关键字个数，关键字不足$D$的在最左边补0），每个关键字有$R$种值，总共进行$D$趟CountingSort。
$$
...
> [!NOTE]
>
> 如果是对整数排序，可以做实验选择最优的R和D的组合（不一定是10进制，R也可以等于16、128、1024等任意值）
...
> 顺序表、链表均可
>
> 空间复杂度：$O(N+R)$，同CountingSort
>
> 时间复杂度：$O(D*(N+R))$，$D$倍的CountingSort
>
> 当N特别大、R和D不太大的时候，RadixSort就会比任何Comparison Sort都优秀

### MSD Radix Sort

在LSD Radix Sort中如果关键字长度D不均（比如`[114514, 0]`，就得让0跟着114514进行6次分配+收集），会浪费很多时间。从高位到低位的比较可以减少很多趟。

![MSDRadixSort](<./img/MSDRadixSort.jpg>)

> 稳定的
>
> 最坏空间复杂度：$O(N+DR)$，$DR$是$R$次深度为$D$的递归
>
> 最好时间复杂度：$O(N+R)$，只进行依次CountingSort
>
> 最坏时间复杂度：$O(D*(N+R))$，同LSD Radix Sort的最坏情况
...
结合Non-Comparison Sort和Comparison Sort的思想，按某种规则（比如mod取余）分到不同的桶内，每个桶内再利用Comparison Sort排序（也可以嵌套多层桶，像多级页表一样）

利用Non-Comparison Sort的indexing减少比较的次数
...
![ADT-Implementation](<./img/ADT-Implementation.jpg>)

![Search-Sort-WrapUp](<./img/Search-Sort-WrapUp.jpg>)
